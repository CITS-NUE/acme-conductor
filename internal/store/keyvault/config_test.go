...
func TestParseConfig(t *testing.T) {
	...
	// The credential defaults to DefaultAzureCredential when omitted.
	if c, err := ParseConfig(json.RawMessage(`{"vaultURL": "https://kv-acme-dev.vault.azure.net"}`)); err != nil || c.Credential != CredentialDefault {
		t.Fatalf("default credential: %+v, %v", c, err)
	}
