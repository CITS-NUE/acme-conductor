...
(Phase 6 — see [`docs/architecture.md`](docs/architecture.md#roadmap)) and
...
- **No cloud SDK in Conductor core.** Azure/AWS/GCP SDK imports belong only
  ...
  and `internal/conductor/launchers` only. What a launcher adapter needs
  to build is its own `Deps` type in its own package; the composition
  layer hands every adapter the same generic `launchers.BuildDeps`
