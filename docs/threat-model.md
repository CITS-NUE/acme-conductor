...
- **Concurrency control covers only runs the Conductor launches (T7).**
  Within one Conductor, at most one run per target is active and a stale
  run is cancelled before it starts. Outside it — a Runner started by
