...
| T9 | Denial of service via oversized/hostile documents | A very large or deeply-nested `JobSpec`/`Result` document is submitted to exhaust memory or CPU during decoding. | Resource exhaustion on the Conductor or Runner. | `decodeStrict` reads through an `io.LimitReader` capped at `MaxDocumentSize` (64 KiB) before attempting to decode anything, and rejects the document outright if that cap is exceeded — so a hostile document cannot grow unbounded before validation even starts. | Existing (`pkg/api/v1alpha1/decode.go`) |
