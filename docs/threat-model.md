...
- **ACME アカウントの資材**．External Account Binding（EAB）の HMAC 鍵を含む．
  EAB は ACME の `newAccount` にのみ使う起動用の資格情報であり，登録済みの
  アカウントによるその後の発行・更新には関与しない（[ADR 0022](adr/0022-encrypted-eab-provisioning-and-account-generations.md)）．
- **Runner の account-provisioning 秘密鍵（X25519）**．EAB を Conductor の
  GUI から Runner まで機密に運ぶための，署名鍵とは別の鍵．Runner の一時領域
  ではなく設定として恒久的に存在するが，DNS／Store の資格情報とは異なり
  ACME/DNS/Store のいかなる操作も認可しない ―― 開けられるのは Runner 自身に
  宛てて封じられた EAB の暗号文だけである．
...
- **JobSpec が制御するコストのレバー．** `policy.renewBeforeDays` と
  ...
  ことは未対策のままである．
- **侵害された Conductor は EAB プロビジョニングの公開鍵を偽装できる（T1，
  [ADR 0022](adr/0022-encrypted-eab-provisioning-and-account-generations.md)）．**
  `GET /account-provisioning/key` が返す `keyId`／`publicKey` は Conductor
  自身の宣言であり，GUI はそれを信じてブラウザ内で EAB を封じる．T15 が
  `/ui/config` の OIDC 発行者について述べる残存リスクと同じ形で，Conductor
  の侵害はこの鍵も攻撃者のものへ差し替えられる．暗号化そのものはこの
  差し替えを検出できない．唯一の対策は運用上のものである: 表示された
  `keyId` を `acme-runner provisioning-keygen` が生成時に出力した `keyId`
  と，Conductor を経由しない経路で比較する．
